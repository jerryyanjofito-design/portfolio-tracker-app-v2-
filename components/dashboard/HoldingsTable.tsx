...
export default function HoldingsTable({ holdings, onDeleteHolding, onRefreshPrice, onBuySell }: HoldingsTableProps) {
  ...
  return (
    <div className="grid-holdings">
      {/* Column Headers */}
      <div className="grid grid-cols-7 gap-4 px-5 pb-3 text-[11px] uppercase text-tertiary font-medium tracking-wider">
        <div>Asset</div>
      ...
      {holdings.map((holding) => {
        ...
        return (
          <div
            key={holding.id}
            className="hover-reveal-row grid grid-cols-7 gap-4 items-center px-5 py-4"
          >
            {/* Asset Column */}
            <div>
              <p className="text-[17px] font-bold text-primary">{holding.ticker}</p>
              <p className="text-[12px] text-secondary">{holding.name}</p>
            </div>
